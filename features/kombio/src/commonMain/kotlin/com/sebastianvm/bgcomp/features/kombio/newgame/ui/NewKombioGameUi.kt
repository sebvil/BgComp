...
import com.sebastianvm.bgcomp.features.kombio.newgame.viewmodel.PlayerNameChanged
...
object NewKombioGameUi : Ui<NewKombioGameState, NewKombioGameUserAction> {
    ...
    @Composable
    override fun invoke(state: NewKombioGameState, handle: (NewKombioGameUserAction) -> Unit, modifier: Modifier) {
        Scaffold(
            ...
            Column(
                ...
                LazyColumn(
                    ...
                    item {
                        Card(modifier = Modifier.fillMaxWidth()) {
                            ...
                            ListItem(
                                ...
                                supportingContent = { Text(UiString(Res.string.points_mode_description)) },
                                leadingContent = {
                                    RadioButton(
                            ...
                            ListItem(
                                ...
                                supportingContent = { Text(UiString(Res.string.rounds_mode_description)) },
                                leadingContent = {
                                    RadioButton(
                        ...
                        when (val mode = state.gameMode) {
                            is GameMode.Points -> {
                                Card(modifier = Modifier.fillMaxWidth()) {
                                    ...
                                    OutlinedTextField(
                                        ...
                                        onValueChange = { newValue ->
                                            ...
                                            newValue.toIntOrNull()?.let { points ->
                                                handle(GameModeDataUpdated(mode.copy(points = points)))
                                            }
                                        },
                                    ...
                                    OutlinedTextField(
                                        ...
                                        onValueChange = { newValue ->
                                            newValue.toIntOrNull()?.let { reentries ->
                                                handle(GameModeDataUpdated(mode.copy(allowedReentries = reentries)))
                                            }
                                        },
                                    ...
                                    ListItem(
                                        headlineContent = { Text(UiString(Res.string.first_to_exceed)) },
                                        supportingContent = { Text(UiString(Res.string.first_to_exceed_description)) },
                                        leadingContent = {
                                            RadioButton(
                                                selected = mode.winOnFirstOverflow,
                                                onClick = { handle(GameModeDataUpdated(mode.copy(winOnFirstOverflow = true))) }
                                            )
                                        }
                                    )

                                    ListItem(
                                        headlineContent = { Text(UiString(Res.string.last_to_reach)) },
                                        supportingContent = { Text(UiString(Res.string.last_to_reach_description)) },
                                        leadingContent = {
                                            RadioButton(
                                                selected = !mode.winOnFirstOverflow,
                                                onClick = { handle(GameModeDataUpdated(mode.copy(winOnFirstOverflow = false))) }
                                            )
                                        }
                                    )
                                }
                                    ...
                                    OutlinedTextField(
                                        ...
                                        onValueChange = { newValue ->
                                            ...
                                            newValue.toIntOrNull()?.let { rounds ->
                                                handle(GameModeDataUpdated(mode.copy(rounds = rounds)))
                                            }
                                        },
                    ...
                    item {
                        Card(modifier = Modifier.fillMaxWidth()) {
                            Row(
                                ...
                                Text(
                                    text = stringResource(Res.string.players_count, state.playerNames.size),
                                    style = BgCompTheme.typography.titleMedium
                                )
                    ...
                    itemsIndexed(
                        ...
                        val text = remember { mutableStateOf(playerName) }
                        val focusManager = LocalFocusManager.current
                        OutlinedTextField(
                            value = text.value,
                            onValueChange = { newName ->
                                text.value = newName
                                handle(PlayerNameChanged(index, newName))
                            },
                ...
                Button(
                    ...
                    modifier = Modifier.fillMaxWidth(),
                    enabled = state.playerNames.all { it.isNotBlank() }
                )
            }
        }
    }
