...
data class KombioGame(
    val players: SerializableList<PlayerState> = persistentListOf(),
    val gameMode: GameMode? = null,
    val rounds: SerializableList<Round> = persistentListOf(),
        ...
        get() = when (gameMode) {
            is GameMode.Points -> {
                ...
                when {
                    ...
                    gameMode.winOnFirstOverflow -> activePlayers.any { it.totalScore >= gameMode.points }
...
data class Round(
    val roundNumber: Int,
    val playerScores: SerializableList<PlayerRoundScore>
)
...
fun calculateRoundScores(
    ...
    val didKombioCallerWin = playersWithMinPoints == 1 && handPoints[kombioCallerIndex] == minHandPoints

    val penalties = handPoints.mapIndexed { index, _ ->
        when {
    ...
    return handPoints.indices.map { index ->
